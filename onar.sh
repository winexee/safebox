#!/usr/bin/env bash
# SafeBox v1.7.3 - Cinnamon Session Crash Fix & Full Classic UI
set -e

# 1. guest-init: Cinnamon-Session Başlatıcı Düzeltmesi (Siyah Ekran Çökmesini Engeller)
...
# Cinnamon tam oturumunu başlat (Yedek olarak cinnamon --replace içerir)
if which cinnamon-session >/dev/null 2>&1; then
    exec cinnamon-session
elif which cinnamon >/dev/null 2>&1; then
    cinnamon --replace 2>/dev/null &
    wait $!
else
    # Fallback (Cinnamon bulunamazsa sistemin çökmesini engeller)
    xfwm4 &
    pcmanfm --desktop &
    tint2 &
    wait
...
# 2. GUI: Tam Orijinal Klasik Arayüz (developer komutu desteği dahil)
...
# 3. Changelog Güncellemesi (v1.7.3)
cat << 'EOF' > debian/changelog
safebox (1.7.3-1~ppa1~noble1) noble; urgency=medium

  * Cinnamon-session entegrasyonu tamamlandı, başlatma çökmesi giderildi.
  * GUI klasik görünümüne ve orijinal developer moduna döndürüldü.

 -- Mehmet Akif Şahin <mehmetakifsahin500@gmail.com>  Sun, 30 Aug 2026 23:59:00 +0300
EOF

# 4. Derle ve Sisteme Kur
dpkg-buildpackage -us -uc -b
sudo dpkg -i ../safebox_1.7.3-1~ppa1~noble1_all.deb

# 5. GitHub'a Gönder
git add usr/ debian/
git commit -m "Fix Cinnamon black screen launch crash and restore full classic UI (v1.7.3)" || true
git tag -d v1.7.3 2>/dev/null || true
git push origin :refs/tags/v1.7.3 2>/dev/null || true
git tag -a v1.7.3 -m "SafeBox v1.7.3 - Stable Cinnamon Desktop & Classic UI"
git push origin main --tags

echo "✅ SafeBox v1.7.3 kuruldu ve GitHub'a yüklendi!"