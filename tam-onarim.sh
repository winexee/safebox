#!/usr/bin/env bash
# SafeBox v1.7.4 - Orijinal Arayüz ve Cinnamon2D Düzeltmesi
set -e

# 1. guest-init: Cinnamon Çökmesini Önleyen 2D Oturum Başlatıcı
...
# 2. GUI: Tam Orijinal Klasik Düzen (v1.6.4 Birebir Tasarımı)
...
# 3. Changelog Güncellemesi (v1.7.4)
cat << 'EOF' > debian/changelog
safebox (1.7.4-1~ppa1~noble1) noble; urgency=medium

  * Cinnamon 2D oturumu entegre edildi, 133 başlatma çökmesi giderildi.
  * GUI birebir orijinal v1.6.4 düzenine döndürüldü.

 -- Mehmet Akif Şahin <mehmetakifsahin500@gmail.com>  Mon, 31 Aug 2026 00:05:00 +0300
EOF
...
# 5. Sadece GitHub'a Gönder
git add usr/ debian/
git commit -m "Restore exact original classic UI & fix Cinnamon 2D session launch (v1.7.4)" || true
git tag -d v1.7.4 2>/dev/null || true
git push origin :refs/tags/v1.7.4 2>/dev/null || true
git tag -a v1.7.4 -m "SafeBox v1.7.4 - Original UI & Cinnamon 2D"
git push origin main --tags

echo "✅ SafeBox v1.7.4 kuruldu ve GitHub'a yüklendi!"